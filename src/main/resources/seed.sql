...
VALUES
    (gen_random_uuid(), 'minji@example.com', '민지', 'hashed_pw1'),
...
WITH
    u1 AS (SELECT id AS user_id FROM users WHERE email = 'minji@example.com'),
...
-- COMMENTS (댓글)
WITH
    au1 AS (SELECT id FROM users WHERE email = 'minji@example.com'),
...
-- LIKES (좋아요)
WITH
    c1 AS (SELECT id AS comment_id FROM comments WHERE content = '정말 흥미로운 기사네요!'),
    u2 AS (SELECT id AS user_id FROM users WHERE email = 'hoyeon@example.com')
