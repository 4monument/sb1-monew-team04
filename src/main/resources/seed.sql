...
INSERT
INTO "users_interests" ("user_id", "interest_id")
SELECT u1.user_id, i1.interest_id
FROM u1,
     i1
UNION
SELECT u2.user_id, i2.interest_id
FROM u2,
     i2;

-- ARTICLES (기사)
INSERT INTO "articles" ("id", "source", "source_url", "title", "summary", "publish_date")
VALUES (gen_random_uuid(), '테크뉴스', 'https://site.com/ai', '인공지능의 미래', 'AI가 바꾸는 세상에 대한 이야기', now()),
...
INSERT
INTO "articles_views" ("user_id", "article_id")
SELECT u1.user_id, a1.article_id
FROM u1,
     a1
UNION
SELECT u2.user_id, a2.article_id
FROM u2,
     a2;
...
INSERT
INTO "notifications" ("id", "user_id", "resource_id", "resource_type", "content")
SELECT gen_random_uuid(), u1.user_id, c1.resource_id, 'comment', '누군가 내 댓글에 좋아요를 눌렀어요.'
FROM u1,
     c1
UNION
SELECT gen_random_uuid(), u2.user_id, c2.resource_id, 'comment', '새로운 댓글이 달렸어요.'
FROM u2,
     c2;
